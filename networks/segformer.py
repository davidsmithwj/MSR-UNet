...
from networks.MSR import MSRAttention


# from networks.favit import Attention
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, dim, head,mlp_ratio=4., qkv_bias=False,qk_scale = None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 reduction_ratio=1, sr_ratio = 1,token_mlp='mix'):
        super().__init__()
        ...
        self.attn = MSRAttention(dim,num_heads=head, qkv_bias=qkv_bias,qk_scale=qk_scale,
                              attn_drop=attn_drop, proj_drop=drop,sr_ratio=sr_ratio
                                  )
    ...
    def forward(self, x: torch.Tensor, H, W) -> torch.Tensor:
        ...
        # y = x + self.attn1(self.norm1(x), H, W)
        # mx = y + self.ffn(self.norm2(tx), H, W)
